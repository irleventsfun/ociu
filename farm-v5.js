const PIPELINE = [
    ...
    phases:[
      {
        ...
        subtiers:[
          ...
          },
          {
            ...
            summary:'A separate podcast project that supports the Search while becoming its own sponsor-ready media property.',
            actions:[
            ...
            feeds:'The Search, EM:Dirt, Monday Morning Leadership, diagnostics, sponsors, and Perry’s modern authority platform.'
          },
          {
  ...
  {
    id:'t1c',label:'T1C',short:'Business Solutions',summary:'The done-for-you implementation path under an agreed Powered by Perry structure.',
  ...
  {
    id:'t2',label:'Tier 2',short:'Diagnostic + SOS',summary:'The permanent analysis and Business Operating System layer.',
    phases:[{id:'analysis',label:'Analysis',short:'Find the bottleneck',summary:'The $497 diagnostic identifies what is actually choking growth.',proof:'Tier 2 increases the lifetime value of a T1A customer without requiring hundreds of new front-door sales.',subtiers:[{label:'Diagnostic',short:'$497 analysis',summary:'Find the primary constraint and route the owner into the correct next move.',actions:[['Assess the business','Collect operating, sales, leadership, and workflow data.'],['Interpret the bottleneck','Identify the primary constraint rather than producing a random list.'],['Route the next action','Recommend T1C, SOS, Tier 3, community, or advanced support.']],feeds:'T1C, SOS, Tier 3, and premium qualification.'},{label:'SOS',short:'$97 founding-year Business Operating System',summary:'More than CRM: a system designed to coordinate mundane functions, tracking, communication, workflows, and onboarding.',actions:[['Install the operating core','Set up people, tasks, pipelines, communication, and onboarding.'],['Automate repeatable work','Reduce mundane manual functions through workflows and AI.'],['Create the upgrade path','Identify users ready for the $297/month builder tier.']],feeds:'Tier 3, custom AI, deeper implementation, and Farm Builders.'}]}]
  },
  {
    id:'t3',label:'Tier 3+',short:'Builders → Farmhouse → Expansion',summary:'A preview of the destination rather than an overloaded first-year promise.',
    phases:[{id:'future',label:'Future',short:'Grow the unicorns',summary:'Advanced SOS builders, defined onboarding AI, ecosystem leaders, premium Perry retreats, and national chapters.',proof:'The ecosystem is designed to grow sophisticated founders over time—not merely sell an academy.',subtiers:[{label:'SOS Builder',short:'$297/month',summary:'Capacity-bound co-building, advanced workflows, and one clearly scoped onboarding AI agent.',actions:[['Join the builder community','Participate in development, testing, and reusable workflow creation.'],['Build one defined AI workflow','Map and automate a clearly scoped onboarding process.'],['Earn leadership','Contribute, implement, and help others.']],feeds:'Farm Builders, Farmhouse retreats, Enterprise work, and chapter leadership.'},{label:'Farmhouse & Expansion',short:'Premium Perry + new chapters',summary:'High-level retreats with Perry and chapter growth led by proven ecosystem leaders.',actions:[['Qualify the leader','Select based on implementation and contribution.'],['Protect Perry’s time','Reserve direct access for high-level intellectual, strategic, and mindfulness work.'],['Choose the leader before the city','Expand through proven operators rather than geography alone.']],feeds:'A national pool of increasingly sophisticated Perry-level founders.'}]}]
  }
];

const TIMELINE=[
  {when:'Months 1–3',title:'Phase 1 — Build T1A',text:'Produce the $1 Million Idea media system, 20+ content assets, PM in the AM episode bank, EM:Dirt academy, Pocket AI, guide, website/social reface, and the lead-generation funnel.',chips:['20+ useful media assets','Full podcast episodes','Short-form promos','Academy + Pocket AI','Working lead funnel']},
  ...
  {when:'Months 10–12',title:'Build the Next Levels',text:'Launch a capacity-bound Tier 3 builder group, define onboarding AI scopes, develop Farm Builders, and prepare premium and chapter pathways.',chips:['Tier 3 cap','AI scope','Farm Builders','Premium path']}
];
...
function current(){const tier=PIPELINE[tierIndex],phase=tier.phases[phaseIndex],sub=phase.subtiers[subIndex];return{tier,phase,sub}}
function renderChoices(id,items,selected,formatter,onClick){const el=document.getElementById(id);el.innerHTML=items.map((x,i)=>`<button class="choice ${i===selected?'active':''}" data-i="${i}">${formatter(x)}</button>`).join('');el.querySelectorAll('button').forEach(b=>b.onclick=()=>onClick(+b.dataset.i))}
...
function renderTimeline(){document.getElementById('timelineGrid').innerHTML=TIMELINE.map(x=>`<article class="timeline-card"><div class="timeline-top"><b>${x.title}</b><span>${x.when}</span></div><p>${x.text}</p><div class="deliverables">${x.chips.map(c=>`<span>${c}</span>`).join('')}</div></article>`).join('')}
function renderTOB(){document.getElementById('tob').innerHTML=TRANSLATE_OPERATE_BUILD.map(x=>`<article class="tob-card"><h3>${x[0]}</h3><p>${x[1]}</p></article>`).join('')}
function openPage(id,btn){document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===id));document.querySelectorAll('.nav').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');window.scrollTo({top:0,behavior:'smooth'})}
function bindNav(){document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>openPage(b.dataset.page,b))}

const ECON={t1a:297,mml:197,farm:500,diag:497,sos:97,tier3:3564};
function initEconomics(team){
  const base=document.getElementById('baseSales'),full=document.getElementById('fullRate'),tier2=document.getElementById('tier2Rate');
  [base,full,tier2].forEach(x=>x.oninput=()=>calcEconomics(team));calcEconomics(team)
}
function calcEconomics(team){
  const base=+document.getElementById('baseSales').value,fullPct=+document.getElementById('fullRate').value,t2Pct=+document.getElementById('tier2Rate').value;
  const full=Math.round(base*fullPct/100),t2=Math.round(base*t2Pct/100);
  document.getElementById('baseOut').textContent=base.toLocaleString();document.getElementById('fullOut').textContent=fullPct+'%';document.getElementById('tier2Out').textContent=t2Pct+'%';
  ...
  document.getElementById('econCopy').textContent=`${base.toLocaleString()} T1A buyers create ${money(t1a)} at the front door. ${full.toLocaleString()} modeled full-path customers add ${money(fullIncrement)}, and ${t2.toLocaleString()} additional Tier 2 customers add ${money(t2Increment)}.`;
  document.getElementById('breakdown').innerHTML=`<div class="break-row"><span>T1A front-door sales</span><b>${money(t1a)}</b></div><div class="break-row"><span>${full} full-path customers</span><b>${money(fullIncrement)}</b></div><div class="break-row"><span>${t2} additional Tier 2 customers</span><b>${money(t2Increment)}</b></div><div class="break-row total"><span>Modeled gross ecosystem revenue</span><b>${money(total)}</b></div>${team?`<div class="break-row"><span>Less $200,000 engagement</span><b>${money(total-200000)}</b></div>`:''}`;
}

document.addEventListener('DOMContentLoaded',()=>{renderPipeline();renderTimeline();renderTOB();bindNav();initEconomics(document.body.classList.contains('team-mode'));const detail=document.getElementById('detailToggle');if(detail)detail.onclick=()=>{document.body.classList.toggle('advanced');detail.classList.toggle('active')}});